...
abrasion_values = SALib.sample.morris.sample(problem, 3)
...
def lith_predict(x,
   ...
   # create arrays to hold sediment fluxes
    ...
    plt.plot(x, theta_ig/theta, label = 'igneous')
    plt.plot(x, theta_mtm/theta, label = 'metamorphic')
    plt.plot(x, theta_sed/theta, label = 'sedimentary')
    plt.legend()
    plt.show()
    
    return x, theta_ig, theta_mtm, theta_sed, theta
...
for i, X in enumerate(abrasion_values):
    ...
    x, theta_ig, theta_mtm, theta_sed, theta = lith_predict(x, 
                                                                       q,
                                                                       num_steps,
                                                                       percent_ig, 
                                                                       percent_mtm, 
