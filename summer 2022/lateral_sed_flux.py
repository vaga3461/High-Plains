...
# -*- coding: utf-8 -*-
"""
Created on Tue Jun 21 15:13:01 2022
...
def two_lith_one_sed(x,
    ...
    while cum_time < run_duration:  # <== use a while loop because dt varies by iteration
        ...
        ql[0] = 0
        ql[1:] = ((bedrock_ero[:-1] * gamma) / atr_factor) * (1 - np.exp(-atr_factor * ((1./3.)*x[1:])))
...
TL_ss = [x, dx, Hstar, H, etab, eta, beta_ig, beta_sed, atr_factor, k_ig, k_sed, r, 0.001, 2]
